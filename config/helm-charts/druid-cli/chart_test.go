...
func TestChartRendersDefaultAndCustomValues(t *testing.T) {
	...
	for _, want := range []string{
		...
		"--worker-callback-url=http://druid-cli.default.svc.cluster.local:8083",
		"--worker-daemon-url=http://druid-cli.default.svc.cluster.local:8081",
		"--public-jwks-url=http://druid-cli.default.svc.cluster.local:8082/.well-known/jwks.json",
		"name: management",
		"name: public",
		"name: worker",
	...
	customManifest := helmTemplate(t,
		...
		"--set", "runtime.workerCallbackUrl=https://worker-callback.example.test",
		"--set", "runtime.workerDaemonUrl=https://worker-daemon.example.test",
	...
	for _, want := range []string{
		...
		"--worker-callback-url=https://worker-callback.example.test",
		"--worker-daemon-url=https://worker-daemon.example.test",

...
func TestProcedureResourceNamePrefixesNumericRuntimeID(t *testing.T) {
	...
	if got, want := procedureResourceName(root, "start", 1), "d-0636a354-start-1"; got != want {
		t.Fatalf("procedureResourceName = %s, want %s", got, want)
	}
	if got, want := serviceName(root, "start", "main"), "d-0636a354-start-main"; got != want {
...
func TestProcedureResourceNameKeepsReadableAlphaRuntimeID(t *testing.T) {
	...
	if got, want := procedureResourceName(root, "start", 0), "deployment-start-0"; got != want {
		t.Fatalf("procedureResourceName = %s, want %s", got, want)
	}
}

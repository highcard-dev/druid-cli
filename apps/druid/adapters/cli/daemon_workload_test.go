...
func (a *recordingWorkloadAuthenticator) AuthenticateWorkload(_ context.Context, token string) (ports.RuntimeWorkloadIdentity, error) {
	a.called = true
	a.token = token
	return ports.RuntimeWorkloadIdentity{Kind: "ui-publish", RuntimeID: "runtime-a"}, nil
}
...
func TestUnsafeModeStillAuthenticatesPresentedWorkloadToken(t *testing.T) {
	...
	app.Post("/api/v1/scrolls/runtime-a/ui/packages/private/prepare", func(c *fiber.Ctx) error {
		identity := c.Locals("druid-workload-identity").(ports.RuntimeWorkloadIdentity)
		return c.SendString(identity.Kind)
	})

	request := httptest.NewRequest("POST", "/api/v1/scrolls/runtime-a/ui/packages/private/prepare", nil)
	...
	if response.StatusCode != 200 || string(body) != "ui-publish" || !authenticator.called || authenticator.token != "projected-token" {
...
func TestUnsafeModeRetainsHeaderFallbackWithoutToken(t *testing.T) {
	...
	defer response.Body.Close()
	body, _ := io.ReadAll(response.Body)
	if response.StatusCode != 200 || string(body) != "ui-publish:runtime-a" || authenticator.called {
		t.Fatalf("status=%d body=%q authenticator=%#v", response.StatusCode, body, authenticator)
	}
}
